...
class BoilerplatePreferencesAddKeymapOperator(bpy.types.Operator):
    """Add key map item"""
...
class BoilerplatePreferencesDefaults():
    hello = "Hello world!"

defaults = BoilerplatePreferencesDefaults()

class BoilerplatePreferences(bpy.types.AddonPreferences):
    ...
    def draw_keymaps(self, layout):
        ...
        subcol.operator(BoilerplatePreferencesAddKeymapOperator.bl_idname, text="Add New", text_ctxt=i18n_contexts.id_windowmanager, icon='ADD')
...
def get_prefs():
    ...
    if prefs:
        return prefs
    return BoilerplatePreferencesDefaults()
...
def register_keymaps():
    ...
    keyconfig_init_from_data(kc_addonconf, [
         (
            ...
            {"items": [
                ("object.empty_operator", {"type": 'F5', "value": 'PRESS'},
                 {"properties": [("example_prop", False),],
                  "active":True}),
            ]},
        ),
    ])
